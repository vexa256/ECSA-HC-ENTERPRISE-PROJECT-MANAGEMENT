...
@php
    ...
    function validatePerformanceSummary($summary) {
        ...
        if (!isset($summary) || !is_array($summary)) {
            return [
                ...
                'clusters_data' => [] // Added for all clusters mode
            ];
        }
        ...
        // Validate clusters data for all clusters mode
        if (!isset($summary['clusters_data']) || !is_array($summary['clusters_data'])) {
    ...
    // Set flag for all clusters mode - this is always true for the V2_ALL controller
    $usingAllClusters = true;

    // Get cluster names from the ClusterNames variable if available
    $clusterNames = isset($ClusterNames) ? $ClusterNames : [];
@endphp

<div class="dashboard-header">
    <div class="dashboard-header-content">
        <h2 class="ios-header">All Clusters Performance Dashboard</h2>

        <div class="flex flex-wrap mb-4">
            <div class="ios-selected-pill">
                <i class="iconify mr-1" data-icon="heroicons-solid:collection"></i>
                Clusters: All Clusters (Combined)
            </div>
        ...
        <div class="ios-tabs">
            ...
            <div class="ios-tab" data-tab="clusters">Cluster Breakdown</div>
        </div>
    </div>
</div>
...
@if(isset($validatedPerformanceSummary) && !isset($Error))
  ...
  <div class="ios-tab-content active" id="overview-content">
      ...
      @if(isset($validatedPerformanceSummary['data_completeness']) && $validatedPerformanceSummary['data_completeness'] < 50)
          <div class="ios-warning mb-6">
              <div class="flex">
                  <div class="flex-shrink-0">
                      <i class="iconify text-yellow-500" data-icon="heroicons-solid:exclamation-triangle"></i>
                  </div>
                  <div class="ml-3">
                      <h3 class="ios-text font-bold">Data Completeness Warning</h3>
                      <p class="ios-text">
                          Only {{ number_format($validatedPerformanceSummary['data_completeness'], 1) }}% of indicators have data.
                          Performance scores may not be representative of actual performance.
                      </p>
                  </div>
              </div>
          </div>
      ...
      <div class="ios-grid ios-grid-4 mb-6">
          <div class="ios-card">
              <div class="ios-summary-card summary-card-blue">
                  ...
                  @if(isset($validatedPerformanceSummary['score_note']))
                      <div class="ios-text-xs mt-2 text-gray-600">{{ $validatedPerformanceSummary['score_note'] }}</div>
                  @endif
              </div>
          </div>
          ...
          <div class="ios-card">
              <div class="ios-summary-card summary-card-green">
                  ...
                  <div class="ios-summary-card-subtitle">Across all clusters</div>
              </div>
          </div>
      ...
      <div class="ios-card p-4 mb-6">
          <h3 class="ios-subheader">Data Completeness</h3>
          <div class="mt-2">
              <div class="flex justify-between mb-1">
                  <span class="ios-text-small">Indicators with Data</span>
                  <span class="ios-text-small font-semibold">
                      {{ $validatedPerformanceSummary['indicators_with_data'] }} / {{ $validatedPerformanceSummary['total_indicators'] }}
                      ({{ number_format(safePercentage($validatedPerformanceSummary['indicators_with_data'], $validatedPerformanceSummary['total_indicators']), 1) }}%)
                  </span>
              </div>
              <div class="ios-progress-container">
                  @php
                      $completenessPercentage = safePercentage($validatedPerformanceSummary['indicators_with_data'], $validatedPerformanceSummary['total_indicators']);
                      $completenessColor = $completenessPercentage < 20 ? 'ios-progress-bar-red' :
                                          ($completenessPercentage < 50 ? 'ios-progress-bar-yellow' :
                                          ($completenessPercentage < 80 ? 'ios-progress-bar-light-green' : 'ios-progress-bar-dark-green'));
                  @endphp
                  <div class="{{ $completenessColor }}" style="width: {{ max(5, min(100, $completenessPercentage)) }}%"></div>
              </div>
          </div>
          <div class="mt-4 ios-text-small">
              <p>
                  @if($completenessPercentage < 20)
                      <span class="text-red-600 font-semibold">Critical:</span> Data completeness is extremely low. Performance assessments are not reliable.
                  @elseif($completenessPercentage < 50)
                      <span class="text-yellow-600 font-semibold">Warning:</span> Data completeness is low. Performance scores should be interpreted with caution.
                  @elseif($completenessPercentage < 80)
                      <span class="text-green-600 font-semibold">Moderate:</span> Data completeness is acceptable but could be improved for more accurate assessments.
                  @else
                      <span class="text-green-800 font-semibold">Good:</span> Data completeness is high, providing a reliable basis for performance assessment.
                  @endif
              </p>
          </div>
      </div>
                  ...
                  <tbody>
                      @foreach($Insights['critical_indicators'] as $indicator)
                          <tr>
                              ...
                              <td>
                                  @if(isset($indicator['clusters']) && is_array($indicator['clusters']))
                                      {{ implode(', ', array_slice($indicator['clusters'], 0, 2)) }}
                                      @if(count($indicator['clusters']) > 2)
                                          <span class="ios-text-xs">+{{ count($indicator['clusters']) - 2 }} more</span>
                                      @endif
                                  @else
                                      {{ safeGet($indicator, 'cluster_name', 'Unknown') }}
                                  @endif
                              </td>
                              <td>{{ safeGet($indicator, 'target_year_range', 'Not set') }}</td>
                              <td>
                                  <div class="flex items-center">
                                      <span class="ios-badge status-not-performing mr-2">
                                          {{ number_format(safeFloat(safeGet($indicator, 'lowest_score', safeGet($indicator, 'score', 0))), 1) }}%
                                      </span>
                                  </div>
                              </td>
                          </tr>
      ...
      <div class="flex flex-wrap gap-4 mb-6">
          <a href="{{ route('V2_ALL_performance.report.generate', ['timeline_id' => $TimelineID ?? 0]) }}" class="ios-button">
              <i class="iconify mr-1" data-icon="heroicons-solid:document-report"></i>
              Generate Report
          </a>

          <a href="{{ route('V2_ALL_performance.timeline.selection') }}" class="ios-button ios-button-secondary">
              <i class="iconify mr-1" data-icon="heroicons-solid:calendar"></i>
              Change Timeline
          </a>
      </div>
                              ...
                              @foreach($objective['indicators'] as $data)
                                  @php
                                      ...
                                      if(isset($data['cluster'])) {
                                          $clusterName = safeGet($data['cluster'], 'Cluster_Name', 'Unknown Cluster');
                                      }
                                      ...
                                      if ($hasTarget && $hasPerformance) {
                                          ...
                                          if ($targetValueFloat > 0) {
                                              $calculatedPercentage = ($actualValueFloat / $targetValueFloat) *
100;
      ...
      @else
          <div class="ios-card ios-empty-state">
              ...
              <p class="ios-empty-state-text">
                  There are no performance indicators available for this timeline.
              </p>
          </div>
      @endif
  </div>

  <!-- Insights Tab Content - Add Data Quality Section -->
  <div class="ios-tab-content" id="insights-content">
      @if(isset($Insights) && !empty($Insights))
          <!-- Add Data Quality Section at the top -->
          @if(isset($Insights['data_quality']) && !empty($Insights['data_quality']))
              <div class="ios-card p-4 mb-6 ios-data-quality-card" style="border-left: 4px solid #ffcc00;">
                  <h3 class="ios-subheader">Data Quality Assessment</h3>
                  <ul class="list-disc pl-5 space-y-2">
                      @foreach(safeGet($Insights, 'data_quality', []) as $insight)
                          <li class="ios-text">{{ $insight }}</li>
                      @endforeach
                  </ul>
              </div>
          @endif
          ...
          <div class="ios-card p-4 mt-6">
              <h3 class="ios-subheader">Cluster Comparisons</h3>
              @if(!empty(safeGet($Insights, 'cluster_comparisons', [])))
                  <ul class="list-disc pl-5 space-y-2">
                      @foreach(safeGet($Insights, 'cluster_comparisons', []) as $comparison)
                          <li class="ios-text">{{ $comparison }}</li>
                      @endforeach
                  </ul>
              @else
                  <p class="ios-text">No cluster comparison data available for this reporting period.</p>
              @endif
          </div>
      ...
      @else
          <div class="ios-card ios-empty-state">
              ...
              <p class="ios-empty-state-text">
                  There are no insights available for this timeline.
              </p>
          </div>
      @endif
  </div>
  ...
  <div class="ios-tab-content" id="strategic-content">
      ...
      @else
          <div class="ios-card ios-empty-state">
              ...
              <p class="ios-empty-state-text">
                  There are no strategic objectives available for this timeline.
              </p>
          </div>
      @endif
  </div>
  ...
  <div class="ios-tab-content" id="clusters-content">
      @if(isset($validatedPerformanceSummary['clusters_data']) && !empty($validatedPerformanceSummary['clusters_data']))
          <div class="ios-card p-4 mb-6">
              <h3 class="ios-subheader">Performance by Cluster</h3>
              <div class="ios-chart-container" id="clusterComparisonChart">
                  <!-- Chart will be rendered here -->
              </div>
              ...
              <div class="mt-4">
                  <table class="ios-table">
                      ...
                      <tbody>
                          @foreach($validatedPerformanceSummary['clusters_data'] as $clusterID => $clusterData)
                              <tr>
                                  <td>{{ $clusterData['name'] }}</td>
                                  <td>{{ $clusterData['indicators'] }}</td>
                                  <td>{{ $clusterData['indicators_with_data'] }}</td>
                                  <td>
                                      @if(isset($clusterData['average_score']) && $clusterData['average_score'] > 0)
                                          {{ number_format($clusterData['average_score'], 1) }}%
                                      @else
                                          N/A
                                      @endif
                                  </td>
                                  <td>
                                      @if(isset($clusterData['average_score']) && $clusterData['average_score'] > 0)
                                          @if($clusterData['average_score'] < 10)
                                              <span class="ios-badge status-not-performing">Not Performing</span>
                                          @elseif($clusterData['average_score'] < 50)
                                              <span class="ios-badge status-in-progress">In Progress</span>
                                          @elseif($clusterData['average_score'] < 90)
                                              <span class="ios-badge status-on-track">On Track</span>
                                          @elseif($clusterData['average_score'] <= 100)
                                              <span class="ios-badge status-met">Met</span>
                                          @else
                                              <span class="ios-badge status-over-achieved">Over Achieved</span>
                                          @endif
      ...
      @else
          <div class="ios-card ios-empty-state">
              <i class="iconify ios-empty-state-icon" data-icon="heroicons-solid:collection"></i>
              <h3 class="ios-empty-state-title">No Cluster Data Available</h3>
              <p class="ios-empty-state-text">
                  There is no individual cluster data available for comparison.
              </p>
          </div>
      @endif
  </div>
@else
  <div class="ios-card ios-empty-state">
      ...
      <p class="ios-empty-state-text">
          There is no performance data available for this timeline.
      </p>
  </div>
@endif
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
      ...
      @if(isset($validatedPerformanceSummary) && !isset($Error))
          ...
          const clusterChartEl = document.getElementById('clusterComparisonChart');
          if (clusterChartEl) {
              try {
                  const clustersData = @json($validatedPerformanceSummary['clusters_data'] ?? []);
                  const clusterLabels = [];
                  const clusterScores = [];
                  const clusterColors = [];

                  // Process cluster data
                  Object.entries(clustersData).forEach(([clusterId, data]) => {
                      if (data.average_score !== null && data.average_score !== undefined) {
                          clusterLabels.push(data.name);
                          clusterScores.push(parseFloat(data.average_score));

                          // Determine color based on score
                          if (data.average_score < 10) {
                              clusterColors.push('#ff3b30'); // Not Performing
                          } else if (data.average_score < 50) {
                              clusterColors.push('#ffcc00'); // In Progress
                          } else if (data.average_score < 90) {
                              clusterColors.push('#34c759'); // On Track
                          } else if (data.average_score <= 100) {
                              clusterColors.push('#30d158'); // Met
                          } else {
                              clusterColors.push('#af52de'); // Over Achieved
                          }
                      }
                  });
                  ...
                  const clusterChart = new ApexCharts(clusterChartEl, clusterChartOptions);
                  clusterChart.render();
              } catch (error) {
                  console.error('Error rendering cluster comparison chart:', error);
                  clusterChartEl.innerHTML = '<div class="ios-empty-state"><p>Error rendering chart</p></div>';
              }
          }
      @endif
          ...
          darkModeToggle.addEventListener('click', () => {
              ...
              if (window.ApexCharts) {
                  try {
                      ...
                      ApexCharts.exec('clusterComparisonChart', 'updateOptions', {
                          theme: {
                              mode: document.documentElement.classList.contains('dark') ? 'dark' : 'light'
                          }
                      });
