...
<div class="ios-card p-4 mb-6">
    ...
    @if(isset($Cluster))
        <div class="ios-selected-pill">
            ...
            Selected Cluster: {{ $Cluster->Cluster_Name }}
        </div>
    @endif

    <p class="text-gray-600 mb-4">
        Please select a reporting timeline to view performance indicators and metrics.
    </p>
</div>

@if(isset($Timelines) && count($Timelines) > 0)
<form action="{{ route('performance.timeline.process', ['cluster_id' => $Cluster->ClusterID]) }}" method="POST">
    @csrf
        ...
        @foreach($Timelines as $timeline)
            <label class="ios-list-item cursor-pointer">
                <div class="flex items-center">
                    ...
                    <div>
                        <!-- Hidden cluster_id is optional now because it's in the URL -->
...
<div class="ios-info-card" style="display: none">
    <div class="flex items-start">
        ...
        <div>
            <h3 class="ios-info-title">About Reporting Timelines</h3>
            <p class="ios-info-text">
                Reporting timelines represent periods for which performance data is collected and analyzed.
                Select a timeline to view the performance metrics for the chosen cluster during that period.
            </p>
        </div>
    </div>
</div>
